...
int main() {
  ...
  auto fpsFrameTime = duration_cast<std::chrono::milliseconds> (fpsStartTime - fpsEndTime);
	...
	char ch;
	bool plWin;
  const square defaultSqr = {static_cast<int>(w.ws_col/2), static_cast<int>(w.ws_row/2), 1, 1, 4, 2};
  ...
  //fpsEndTime = fpsStartTime = std::chrono::high_resolution_clock::now();
	...
	auto movePlRand = std::bind(std::uniform_int_distribution<>(-4,4),std::default_random_engine());
	auto winPlRand = std::bind(std::uniform_int_distribution<>(0,1),std::default_random_engine());
