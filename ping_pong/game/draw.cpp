...
void drawScreen(const square &sqr, const player &rightPl, const player &leftPl,
                int windowWidth, int windowHeight, int fps,
                const char BACKGROUND_CHAR, const prediction &pred,
                bool &redrawing) {
    int cursorX = 0;
    int cursorY = 0;
    // const char *screen_arr      = new char[windowHeight][windowWidth];
    // const char *screen_arr_old  = new char[windowHeight][windowWidth];
    static std::vector<std::vector<char>> screen_arr(
        windowHeight, std::vector<char>(windowWidth, 0));
    static std::vector<std::vector<char>> screen_arr_old(
        windowHeight, std::vector<char>(windowWidth, 0));

    for (int y = 0; y < windowHeight; y++) {
        for (int x = 0; x < windowWidth; x++) {
            if ((screen_arr[y][x] = getChar(
                     9, static_cast<int>(sqr.posX - sqr.sizeX),
                     static_cast<int>(sqr.posY - sqr.sizeY), x, y)) != ' ')
                continue;

            if ((x < leftPl.width &&
                 (y >= leftPl.pos && y < leftPl.pos + leftPl.height)) ||
                (x >= windowWidth - rightPl.width &&
                 (y >= rightPl.pos && y < rightPl.pos + rightPl.height))) {
                screen_arr[y][x] = '@';
                continue;
            }

            if ((screen_arr[y][x] =
                     getChar(':', static_cast<int>(windowWidth / 2) - 1, 1, x,
                             y)) != ' ')
                continue;
            if ((screen_arr[y][x] = getChar(
                     leftPl.score + '0', static_cast<int>(windowWidth / 2) - 6,
                     1, x, y)) != ' ')
                continue;
            if ((screen_arr[y][x] = getChar(
                     rightPl.score + '0', static_cast<int>(windowWidth / 2) + 2,
                     1, x, y)) != ' ')
                continue;

            if (pathFind(pred, x, y) && ((sqr.speedX > 0 && rightPl.showPred) ||
                                         (sqr.speedX < 0 && leftPl.showPred))) {
                screen_arr[y][x] = 'X';
                continue;
            }
        ...
        for (int y = 0; y < windowHeight; y++) {
            for (int x = 0; x < windowWidth; x++) {
                if (screen_arr[y][x] != screen_arr_old[y][x]) {
                    if (cursorY == y && !(cursorY == 0 && cursorX == 0) &&
                        x == cursorX + 1) {
                        std::cout << screen_arr[y][x];
                        cursorX++;
                    } else {
    ...
    for (size_t i = 0; i < string_fps.length(); i++) {
        if (screen_arr[0][i] != BACKGROUND_CHAR)
            std::cout << "\033[0;47m";
        else
            std::cout << "\033[0;40m";
    ...
    for (int y = 0; y < windowHeight; y++) {
        ...
        for (int x = 0; x < windowWidth; x++) {
            ...
            for (char ch : text) {
                ...
                if ((tempCh = getChar(
                         ch, drawPosX,
                         static_cast<int>(windowHeight / 2 - height / 2), x,
                         y)) != ' ') {
                    drawCh = tempCh;
                    break;
                }
...
void playerWinScreen(int player, int windowWidth, int windowHeight,
...
void newPointPlayer(int player, int windowWidth, int windowHeight,
                    char background_char) {
    std::string text = "PLAYER ";
    text += std::to_string(player);
    text += "  +1 POINT";
