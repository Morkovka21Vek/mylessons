...
sterling summSterling(sterling ster1, sterling ster2) {
    ...
    sterOut.shillings = static_cast<int>((ster1.pence + ster2.pence) / 12);

    sterOut.pounds = static_cast<int>((sterOut.shillings + ster1.shillings + ster2.shillings) / 20);
    sterOut.shillings = (sterOut.shillings + ster1.shillings + ster2.shillings) % 20;
...
void showSterling(sterling ster) {
    std::cout << "Ответ: £" << ster.pounds << '.' << ster.shillings << '.' << ster.pence << std::endl;
}
