...
void set_curs_star(int x, int y, player &leftPl, player &rightPl,
                   unsigned short contentYsize) {
    ...
    if (y == 4) {
        ...
        if (!change_pred && y == bot_pl_mode) {
            std::cout << "\033[" << contentYsize - 1 + 3 << ';' << x * 20 + 2
                      << 'H' << '*';
            pl.showPred = true;
        } else if (!change_pred) {
            std::cout << "\033[" << contentYsize - 1 + 3 << ';' << x * 20 + 2
...
void start_menu(player &leftPl, player &rightPl, int windowWidth,
    ...
    const char *plmods[] = {"bot", "keyboard", "std", "http"};
    ...
    std::cout << std::left << "Составьте конфигурацию:" << std::endl
              << std::setw(20) << "Player 1(left)" << std::setw(20)
              << "Player 2(right)" << std::endl;

    for (unsigned int i = 0;
         i < static_cast<unsigned int>(sizeof(plmods) / sizeof(char *)); i++) {
        std::cout << "[ ] " << std::setw(16) << plmods[i] << "[ ] "
                  << std::setw(16) << plmods[i] << std::endl;
    }

    std::cout << "\033[32m" << std::setw(20) << "[ ] pred" << std::setw(20)
              << "[ ] pred" << "\033[0m" << std::endl;

    std::cout << std::endl
              << "Используйте стрелки или h(<) j(v) k(^) l(>) для "
                 "перемещения\n\tи _ (пробел) для выбора пункта"
              << std::endl;
    std::cout << "Для выхода из меню(продолжения игры) используйте Enter,\n\tа "
                 "для выхода из игры - q"
              << std::endl;
    ...
    while ((ch = getchar()) != '\n') {
        ...
        case 'Q':
            tcsetattr(STDIN_FILENO, TCSANOW,
                      &oldt);                   // Restore old terminal settings
            fcntl(STDIN_FILENO, F_SETFL, oldf); // Restore old flags
            std::cout << "\033[" << contentYsize + 3 + 6 << ';' << 0 << 'H'
                      << std::endl;
            exit(0);
            break;
        }
