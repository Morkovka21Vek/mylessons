...
void CollisionSystem::handlePaddleCollision(Ball &ball, const Player &player,
                                            const scrsize ws) {
    switch (player.getPlayerposX()) {
    case Playerpos::left:
        handleLeftPaddleCollision(ball, player, ws);
        break;
    case Playerpos::right:
        handleRightPaddleCollision(ball, player, ws);
        break;
    }
}

void CollisionSystem::handleLeftPaddleCollision(Ball &ball,
                                                const Player &player,
                                                const scrsize ws) {
    if (ball.getX() <= player.calcX(ws) + player.getWidth() &&
        ball.getY() + ball.getHeight() >= player.getPos() &&
        ball.getY() <= player.getPos() + player.getHeight()) {
        ball.setPositiveX();
    }
}

void CollisionSystem::handleRightPaddleCollision(Ball &ball,
                                                 const Player &player,
                                                 const scrsize ws) {
...
void CollisionSystem::handleGoalCollision(Ball &ball, const scrsize ws,
                                          ScoreBoard &scboard, Player &leftPl,
                                          Player &rightPl) {
    if (ball.getX() < 0) {
...
void CollisionSystem::resetPos(Ball &ball, Player &leftPl, Player &rightPl,
                               const scrsize ws) {
    ball.reset(ws);
    leftPl.reset(ws);
    rightPl.reset(ws);
