...
int setting_car() {
    ...
    if (car_serial_port < 0) {
        fprintf(stderr,
                "%s: Не удалось открыть устройство(машина)   (ERROR: %d; Line: "
                "%d): %s\n",
...
int setting_joy() {
    ...
    if (joy_fd < 0) {
        fprintf(stderr,
                "%s: Не удалось открыть устройство(джойстик) (ERROR: %d; Line: "
                "%d): %s\n",
                __FILE__, errno, __LINE__, strerror(errno));
    }
...
int send_data(mode_joy joy_mode, const int joy_fd, const int car_serial_port) {
    ...
    struct js_event e;
    static int x_axis = 0, y_axis = 0, old_x_axis_map = -1, old_y_axis_map = -1;
    ...
    if (read(joy_fd, &e, sizeof(e)) == sizeof(e)) {
        ...
        case rudder:
            ...
            case 0:
                do {
                    ...
                    x_axis = ((float)crop_eval_x - (-7000)) / (7000 - (-7000)) *
                                 (32767 - (-32768)) +
                             (-32767);

                    debugf(1, "crop_eval_x: %d real: %d; x_axis: %d; X: %f\n",
                           crop_eval_x, e.value, x_axis,
                           ((float)x_axis + 32768) / (32768 * 2) * 100);
                } while (0);
                break;
            case 3:
            ...
            case 2:
                y_axis =
                    -(32768 - (static_cast<int>(e.value / 2) + (32768 / 2)));
                debugf(2, "real: %d; y_axis: %d\n", e.value, y_axis);
                break;
            }
            break;
        ...
        int x_axis_map =
            static_cast<int>(
                (120 + (x_axis - (-32768)) * (55 - 120) / (32768 - (-32768))) /
                5) *
            5;
        int y_axis_map =
            static_cast<int>((static_cast<float>(-y_axis) / 32768 * 255) / 10) *
            10;
